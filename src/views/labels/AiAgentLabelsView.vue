<template>
  <div class="labels-view">
    <PageHeader
      title="AI Agent Labels"
      subtitle="Manage AI agent labels"
...
function handleCreate() {
  router.push('/dashboard/labels/ai-agent/new')
}

async function handleEdit(label: Label) {
  router.push(`/dashboard/labels/ai-agent/${label.id}`)
}

async function applyFilters() {
  await labelsStore.loadLabelsByCategory('ai_agent', 1, labelsStore.pageSize)
}
...
async function handleDelete(identifier: string) {
  try {
    ...
    await labelsStore.loadLabelsByCategory('ai_agent', targetPage, pageSize)
  } catch (error) {
...
async function handleBulkArchive() {
  ...
  try {
    ...
    await labelsStore.loadLabelsByCategory('ai_agent', labelsStore.pageNum, labelsStore.pageSize)
  } catch (error) {
...
onMounted(() => {
  labelsStore.loadLabelsByCategory('ai_agent', labelsStore.pageNum, labelsStore.pageSize)
})
...
async function handlePageChange(page: number) {
  await labelsStore.loadLabelsByCategory('ai_agent', page, labelsStore.pageSize)
}

async function handlePageSizeChange(pageSize: number) {
  await labelsStore.loadLabelsByCategory('ai_agent', 1, pageSize)
}
</script>
