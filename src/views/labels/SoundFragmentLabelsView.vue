<template>
  <div class="labels-view">
    <PageHeader
      title="Sound Fragment Labels"
      subtitle="Manage sound fragment labels"
...
function handleCreate() {
  router.push('/dashboard/labels/sound-fragment/new')
}

async function handleEdit(label: Label) {
  router.push(`/dashboard/labels/sound-fragment/${label.id}`)
}

async function applyFilters() {
  await labelsStore.loadLabelsByCategory('sound_fragment', 1, labelsStore.pageSize)
}
...
async function handleDelete(identifier: string) {
  try {
    ...
    await labelsStore.loadLabelsByCategory('sound_fragment', targetPage, pageSize)
  } catch (error) {
...
async function handleBulkArchive() {
  ...
  try {
    ...
    await labelsStore.loadLabelsByCategory('sound_fragment', labelsStore.pageNum, labelsStore.pageSize)
  } catch (error) {
...
onMounted(() => {
  labelsStore.loadLabelsByCategory('sound_fragment', labelsStore.pageNum, labelsStore.pageSize)
})
...
async function handlePageChange(page: number) {
  await labelsStore.loadLabelsByCategory('sound_fragment', page, labelsStore.pageSize)
}

async function handlePageSizeChange(pageSize: number) {
  await labelsStore.loadLabelsByCategory('sound_fragment', 1, pageSize)
}
</script>
