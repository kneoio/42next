...
function handleCreate() {
  router.push('/dashboard/labels/script/new')
}

async function handleEdit(label: Label) {
  router.push(`/dashboard/labels/script/${label.id}`)
}

async function applyFilters() {
  await labelsStore.loadLabelsByCategory('script', 1, labelsStore.pageSize)
}
...
async function handleDelete(identifier: string) {
  try {
    ...
    await labelsStore.loadLabelsByCategory('script', targetPage, pageSize)
  } catch (error) {
...
async function handleBulkArchive() {
  ...
  try {
    ...
    await labelsStore.loadLabelsByCategory('script', labelsStore.pageNum, labelsStore.pageSize)
  } catch (error) {
...
onMounted(() => {
  labelsStore.loadLabelsByCategory('script', labelsStore.pageNum, labelsStore.pageSize)
})
...
async function handlePageChange(page: number) {
  await labelsStore.loadLabelsByCategory('script', page, labelsStore.pageSize)
}

async function handlePageSizeChange(pageSize: number) {
  await labelsStore.loadLabelsByCategory('script', 1, pageSize)
}
</script>
