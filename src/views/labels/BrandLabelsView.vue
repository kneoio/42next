...
function handleCreate() {
  router.push('/dashboard/labels/brand/new')
}

async function handleEdit(label: Label) {
  router.push(`/dashboard/labels/brand/${label.id}`)
}

async function applyFilters() {
  await labelsStore.loadLabelsByCategory('brand', 1, labelsStore.pageSize)
}
...
async function handleDelete(identifier: string) {
  try {
    ...
    await labelsStore.loadLabelsByCategory('brand', targetPage, pageSize)
  } catch (error) {
...
async function handleBulkArchive() {
  ...
  try {
    ...
    await labelsStore.loadLabelsByCategory('brand', labelsStore.pageNum, labelsStore.pageSize)
  } catch (error) {
...
onMounted(() => {
  labelsStore.loadLabelsByCategory('brand', labelsStore.pageNum, labelsStore.pageSize)
})
...
async function handlePageChange(page: number) {
  await labelsStore.loadLabelsByCategory('brand', page, labelsStore.pageSize)
}

async function handlePageSizeChange(pageSize: number) {
  await labelsStore.loadLabelsByCategory('brand', 1, pageSize)
}
</script>
