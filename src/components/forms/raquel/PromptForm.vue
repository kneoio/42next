...
import { useRaquelPromptsStore, RAQUEL_PROMPT_TYPES, type RaquelPrompt } from '@/stores/raquelPrompts'
...
const formData = ref<Partial<RaquelPrompt>>({
  title: '',
  description: '',
  prompt: '',
  promptType: 'SONG',
...
const promptTypeOptions = RAQUEL_PROMPT_TYPES.map(t => ({ label: t, value: t }))
...
const showReasoning = ref(localStorage.getItem('raquelPromptForm.showReasoning') === 'true')
...
watch(showReasoning, (val) => localStorage.setItem('raquelPromptForm.showReasoning', String(val)))
...
async function loadDraftOptions() {
  try {
    const result = await raquelApiService.getPagedDictionary<any>('/drafts', 1, 100)
...
async function handleSave() {
  try {
    ...
    router.push('/dashboard/raquel/prompts')
  } catch (error: any) {
...
async function openTestDialog() {
  ...
  if (formData.value.draftId) {
    try {
      const fullDraft = await raquelApiService.getDocument<any>('/drafts', formData.value.draftId)
      if (fullDraft?.content) {
        const vars = await raquelApiService.post<any[]>('/drafts/extract-variables', { code: fullDraft.content })
  ...
  try {
    ...
    if (formData.value.draftId) {
      try {
        const fullDraft = await raquelApiService.getDocument<any>('/drafts', formData.value.draftId)
        if (fullDraft?.content) {
          executedDraft = await raquelApiService.getText('/drafts/test', {
    ...
    const raw = await raquelApiService.getText('/prompts/test', {
...
onMounted(async () => {
  ...
  try {
    ...
    const [langs, masterPrompts] = await Promise.allSettled([
      ...
      raquelApiService.getPagedDictionary<any>('/prompts', 1, 100),
    ])
    ...
    if (isEditing.value) router.push('/dashboard/raquel/prompts')
  } finally {
    loading.value = false
  }
})
